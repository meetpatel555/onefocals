...
function omega_jewelry_store_category_add_form_fields_callback() {
    $omega_jewelry_store_image_id = null; ?>
...
function omega_jewelry_store_custom_create_term_callback($omega_jewelry_store_term_id) {
    // Check if 'category_custom_image_url' exists before using it
    if ( isset( $_REQUEST['category_custom_image_url'] ) ) {
        // Add term meta data
        add_term_meta(
            $omega_jewelry_store_term_id,
            'term_image',
...
function omega_jewelry_store_category_edit_form_fields_callback($ttObj, $taxonomy) {
    $omega_jewelry_store_term_id = $ttObj->term_id;
    $omega_jewelry_store_image = '';
    $omega_jewelry_store_image = get_term_meta( $omega_jewelry_store_term_id, 'term_image', true ); ?>
    <tr class="form-field term-image-wrap">
        <th scope="row"><label for="image"><?php esc_html_e('Image','omega-jewelry-store'); ?></label></th>
        <td>
        <?php if ( $omega_jewelry_store_image ): ?>
            <span id="category_custom_image">
               <img src="<?php echo $omega_jewelry_store_image; ?>" style="width: 100%"/>
            </span>
            <input type="hidden" id="category_custom_image_url" name="category_custom_image_url">
            <span>
...
function omega_jewelry_store_edit_term_callback($omega_jewelry_store_term_id) {
    // Check if 'category_custom_image_url' exists before using it
    if ( isset( $_POST['category_custom_image_url'] ) ) {
        $omega_jewelry_store_image = get_term_meta( $omega_jewelry_store_term_id, 'term_image' );
        if ( $omega_jewelry_store_image ) {
            update_term_meta( 
                $omega_jewelry_store_term_id, 
                'term_image', 
